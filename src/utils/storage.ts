 ...
 */
export type StorageKeys = '@app:user' | '@app:authToken' | '@app:settings' | '@app:preferences';
 ...
 * ```
 */
export async function multiGet(keys: string[]): Promise<[string, any][]> {
  try {
 ...
 * ```
 */
export async function setObject<T extends Record<string, any>>(
  key: string,
  value: T,
): Promise<void> {
  await setItem(key, value);
